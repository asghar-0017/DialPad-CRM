...
const messageRepository = {
  assignMessageToAgentById: async (
    adminId,
    agentId,
    message,
    messageId,
    ...
    try {
      ...
      console.log("Creating message entity with data:", {
        agentId: agent.id,
        adminId,
        message,
        messageId,
  ...
  getSendMessagesFromAdminById: async (agentId, adminId) => {
    try {
      ...
      const AdminMessage = await agentTaskRepository.find({
        where: { agentId },
      });
