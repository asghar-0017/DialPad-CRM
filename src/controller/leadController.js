...
const leadController = {
    ...
    try {
      ...
      if (data.CustomerFeedBack === "followUp" && !data.FollowUpDetail) {
        return res
          ...
          .json({
            message:
              "FollowUpDetail is required when CustomerFeedBack is followUp",
          });
      }
      if (data.CustomerFeedBack === "other" && !data.otherDetail) {
        return res
          ...
          .json({
            message:
              "Other details are required when CustomerFeedBack is other",
          });
      }
  ...
  updateLead: async ( req, res) => {
    try {
      ...
      const lead = await leadService.updateLeadByService({
        data,
        leadId,
        user,
      });
  ...
  saveExcelFileData: async ( req, res) => {
    ...
    try {
      ...
      for (const row of results) {
        ...
        const assignedLead = await leadService.leadCreateService(
          leadData,
          req.user
        );
        if (assignedLead) {
