...
const messageController = {
  sendMessageToAgent: async ( req, res) => {
    try {
      ...
      const data = await messageService.assignMessageToAgent(
        adminId,
        agentId,
        message.message,
        message.messageId,
  ...
  sendMessageToAdmin: async (req, res) => {
    try {
      ...
      message.messageId = messageId();
      const role = req.user.role;
      const data = await messageService.sendMessageToAdminInService(
        agentId,
        adminId,
        message.message,
        message.messageId,
  ...
  getSendMessagesToAdminById: async ( req, res) => {
    try {
      const agentId = req.params.agentId;
      const adminId = req.params.adminId;
      const data = await messageService.getSendMessagesFromAdminById(
        agentId,
        adminId
      );
      if (data === "Data Not Found") {
  ...
  getAllMessages: async (io, req, res) => {
    try {
      ...
      const agentMessages =
        await messageService.getAssignMessagesToAgentById(agentId);
      const adminMessages = await messageService.getSendMessagesFromAdminById(
        agentId,
        adminId
      );
      const allMessages = {
