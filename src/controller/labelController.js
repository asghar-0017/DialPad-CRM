...
const labelController = {
  createLabel: async (req, res) => {
    try {
      ...
      const existSheetId = await sheetRepository.findOne({
  ...
  getLabel: async (req, res) => {
    try {
      ...
      if (existSheetId) {
        ...
        return res
          .status(200)
          .json({ message: "Labels retrieved successfully", data: labels });
      } else {
        return res.status(404).json({ message: "Sheet not found" });
      }
    } catch (error) {
    ...
    try {
      ...
      if (!name || typeof name !== "string") {
        return res
          .status(400)
          .json({
            message: "Invalid input. Please provide a valid label name.",
          });
      }
      ...
      return res
        .status(500)
        .json({ message: "Error updating label", error: error.message });
    }
  },
};
