...
const otherController = {
  ...
  getOtherUpById: async ( req, res) => {
    try {
      ...
      if (other) {
        if (other.role === "admin") {
          const { id, leadId, leadName, phone, email, role, otherDetail } =
            other;
          const data = {
            id,
            leadId,
  ...
  getallSpecificOtherByAgentId: async ( req, res) => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        res
          .status(404)
          .json({ message: `No others found for agentId ${agentId}` });
      }
    } catch (error) {
      console.error("Error fetching others:", error);
      res
  ...
  updateOther: async (req, res) => {
    try {
      ...
      const updatedOther = await otherService.updateOtherById(
        leadId,
        otherData
      );
      if (updatedOther) {
